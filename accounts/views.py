...
from .models import Patient, Doctor, Secretary
    ...
    if request.method=='POST':
        ...
        if pass1==pass2:
            if User.objects.filter(username=email).exists():
                messages.info(request,"La direccion de correo ya esta en uso!")
                ...
                if typ=="Patient":
                    ...
                    return redirect('login')
                if typ=="Secretary":
                    pro=Secretary(user=user,phone=phone,otp=o)
                    pro.save()
                    m='Hey '+fname+'!\n'+'Thank you for registering with the Hospital Management!\n\n'+'Your OTP is :'+o
                    send_mail('Registration Successful!',m,'Hospital Management',[email],fail_silently=True)
                    messages.info(request,"Cuenta Creada Correctamente")
...
def login(request):
    if request.method=='POST':
        ...
        if user is not None:
            ...
            if user.last_name=="Secretary":
                pro=Secretary.objects.filter(user=user).first()
            if pro.verify=='0':
...
def uprofile(request):
    ...
    if request.user.last_name=='Secretary':
        pro=Secretary.objects.filter(user=request.user).first()
...
def Secretaria(request):
    if request.user.last_name == 'Secretary':
        ...
        if pro is not None:
            c = {'pro': pro}
            return render(request, 'accounts/Secretaria.html', c)
        else:
...
@login_required
def profile(request):
    pro = None  # Establecer pro en None por defecto
    if request.user.last_name == 'Doctor':
        pro = Doctor.objects.filter(user=request.user).first()
    elif request.user.last_name == 'Patient':
        pro = Patient.objects.filter(user=request.user).first()
    elif request.user.last_name == 'Secretary':
        pro = Secretary.objects.filter(user=request.user).first()
